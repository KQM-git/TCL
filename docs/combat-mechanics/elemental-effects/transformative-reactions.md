...
* As of patch 2.5, there is a bug when using Electro as the trigger. The Electro aura will not persist after the Electro-Charged ticks, even when the Electro gauge is greater than the Hydro gauge. Refer to the [Evidence Vault](/evidence/combat-mechanics/elemental-effects/transformative-reactions.md#Electro-Charged-hydro-aura-electro-trigger-interaction-is-bugged) for more details.
...
Multiple consecutive Freezees have reduced duration. They follow the following kinematic rules:
...
Thus, extending the above Freezee formula, the generalized Freezee duration formula may be represented by the following:
